...
Dragging a task in (or hitting the ↓ next to it, to send it back down to
the ranked list) actually applies (or removes) an **"Up Next" label** on
the task in Todoist — this is the one place besides marking a task
complete where the app writes back. That means which tasks are in Up Next
is the same everywhere you sign in, not just this browser: label a task
from your phone's Todoist app directly and it'll show up here too.
